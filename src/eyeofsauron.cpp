...
//----------------------
string get_text(const string& str, int i = 0, int count = 0, bool found = false) {
	if (i >= str.length()) return count == 0 ? "correct" : "fix";
	return get_text(str, i+1, count + (str[i]=='|' ? (found ? 1 : -1) : 0), found || str[i]=='(');
}

int main() {
	
	cinstring(s);
	cout << get_text(s) << endl;
