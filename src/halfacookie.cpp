...
//----------------------
bool isvalid(double r, double x, double y) {
	return sqrt(x*x + y*y) <= r;
}

double circlearea(double r, double angle = M_PI) {
	return angle * r * r;
}

double smallarea(double r, double x, double y) {
	double a = sqrt(x*x + y*y);

	double angle = acos(a/r);

	double arcarea = circlearea(r, angle);

	double b = sin(angle) * r;

	double triAreaTotal = a*b;

	return arcarea - triAreaTotal;
}

int main() {

	cindouble(r); cindouble(x); cindouble(y);

	while(!cin.fail()) {
		if (isvalid(r, x, y)) {
			double sarea = smallarea(r, x, y);
			cout << circlearea(r) - sarea << " " << sarea << endl;
		} else cout << "miss" << endl;
		cin >> r >> x >> y;
	}
