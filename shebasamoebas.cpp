...
bool is_amoeba(Vstring& grid, int y, int x) {
	...
	if (grid[y][x] != '#') return false;
		...
		do {
			...
			for(int i = 0 ; i < 8; i++) {
				int tx = x+int( cos(angle*double(i)) * length );
				int ty = y+int( sin(angle*double(i)) * length );
...
int main() {
	...
	cinvector(string, grid, m);
	
	int count = 0;
	for(int i = 0; i < m; i++)
		for(int j = 0; j < n; j++) 
			if (is_amoeba(grid, i, j)) count++;
